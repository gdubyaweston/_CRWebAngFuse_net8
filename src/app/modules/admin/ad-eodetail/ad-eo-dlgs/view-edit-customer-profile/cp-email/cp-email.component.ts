...
@Component({
    selector: 'cp-email',
      standalone: true,
      templateUrl: './cp-email.component.html',
...
export class CPEmailComponent {
    ...
    constructor(
        private _activatedRoute: ActivatedRoute,
            ...
            public _dialogRef: MatDialogRef<CPEmailComponent>,
    ) 
    {        
        this._gfs.showLog('CPEmailComponent', 'constructor', '', null);
        //this._gfs.allowCall = false;
    }

    ngAfterViewInit(): void {
      this._gfs.showLog('CPEmailComponent', 'ngAfterViewInit', '', null);
    }
  
    ngOnInit(): void {
      this._gfs.showLog('CPEmailComponent', 'ngOnInit', '', null);
    ...
    saveEmails(): void {
        if(this.customerData !== null){
            ...
            this._par.saveParticipant(this.setCustomerData(this.customerData)).subscribe({
              next: (result: any) => {
                this._gfs.showLog('CPEmailComponent', 'saveEmails', 'Response:', result);
                if(result.success){
              ...
              error: (err: HttpErrorResponse) => { 
                  this._gfs.showLog('CPEmailComponent', 'saveEmails', 'Error:', err);               
                  if (err.status == 401) {
                      this._ts.signOut();
    ...
    setCustomerData(ep: EscrowParticipant): EscrowParticipant {
      this._gfs.showLog('CPEmailComponent', 'setCustomerData', 'EP:', ep);
