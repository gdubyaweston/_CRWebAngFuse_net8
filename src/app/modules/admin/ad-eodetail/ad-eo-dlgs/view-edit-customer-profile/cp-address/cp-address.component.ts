...
@Component({
    selector: 'cp-address',
      standalone: true,
      templateUrl: './cp-address.component.html',
...
export class CPAddressComponent {
    ...
    constructor(
        private _activatedRoute: ActivatedRoute,
            ...
            public _dialogRef: MatDialogRef<CPAddressComponent>,
    ) 
    {        
        this._gfs.showLog('CPAddressComponent', 'constructor', '', null);
        //this._gfs.allowCall = false;
    }

    ngAfterViewInit(): void {
      this._gfs.showLog('CPAddressComponent', 'ngAfterViewInit', '', null);
    }
  
    ngOnInit(): void {
      this._gfs.showLog('CPAddressComponent', 'ngOnInit', '', null);
    ...
    saveAddresses(): void {
        if(this.customerData !== null){
            ...
            this._par.saveParticipant(this.setCustomerData(this.customerData)).subscribe({
              next: (result: any) => {
                this._gfs.showLog('CPAddressComponent', 'saveAddresses', 'Response:', result);
                if(result.success){
              ...
              error: (err: HttpErrorResponse) => { 
                  this._gfs.showLog('CPAddressComponent', 'saveAddresses', 'Error:', err);               
                  if (err.status == 401) {
                      this._ts.signOut();
    ...
    setCustomerData(ep: EscrowParticipant): EscrowParticipant {
      this._gfs.showLog('CPAddressComponent', 'setCustomerData', 'EP:', ep);
