...
@Component({
    selector: 'edit-released-funds',
      standalone: true,
      templateUrl: './edit-released-funds.component.html',
      encapsulation: ViewEncapsulation.None,
      imports: [NgIf, CommonModule, FormsModule, MatButtonModule, MatDialogModule, 
        MatPaginatorModule, MatSortModule, MatTableModule, MatDatepickerModule, MatFormFieldModule],
})
export class EditReleasedFundsComponent {
    ...
    {        
        this._gfs.showLog('EditReleasdFunds', 'constructor', '', null);
        this._gfs.allowCall = false;
    }

    ngAfterViewInit(): void {
        this._gfs.showLog('EditReleasdFunds', 'ngAfterViewInit', '', null);
        this.getCurrencyCodes();
        this.getFundInfo();
    }
    
    ngOnInit(): void {
        this._gfs.showLog('EditReleasdFunds', 'ngOnInit', '', null);
    }

    setData(eid: number, fid: number): void {
        this._gfs.showLog('EditReleasdFunds', 'setData', '', null);
    ...
    getFundInfo(): void {
        this._gfs.showLog('EditReleasdFunds', 'getFundInfo', '', null);
        ...
        this._fs.getInitialFundingInformation(epdto).subscribe({
          next: (fundinfo: any) => {
            this._gfs.showLog('EditReleasdFunds', 'getFundInfo', 'Respone:', fundinfo);
            if(fundinfo.success){
          ...
          error: (err: HttpErrorResponse) => {
            this._gfs.showLog('EditReleasdFunds', 'getFundInfo', 'Error:', err);
            if(err.status == 401){
                this._ts.signOut();
    ...
    deleteFunds(): void {
        this._gfs.showLog('EditReleasdFunds', 'deleteFunds', '', null);
        let epdto = new EscrowParams();
        epdto.escrowOrderID = this.escrowOrderID;
        epdto.fundsID = this.fundID;
        epdto.fundListType = "Released";
        ...
        this._fs.deleteInitialFunding(epdto).subscribe({
          next: (resp: any) => {
            this._gfs.showLog('EditReleasdFunds', 'deleteFunds', 'Response:', resp);
            if(resp.success){
          ...
          error: (err: HttpErrorResponse) => {
            this._gfs.showLog('EditReleasdFunds', 'deleteFunds', 'Error:', err);
            if(err.status == 401){
                this._ts.signOut();
                this._rtr.navigate(['/sign-in']);
            }
    ...
    saveFunds(): void {
        this._gfs.showLog('EditReleasdFunds', 'saveFunds', '', null);
        let dd = this.buildDto();
        this._fs.saveInitialFunding(dd).subscribe({
          next: (resp: any) => {
            this._gfs.showLog('EditReleasdFunds', 'saveFunds', 'Response:', resp);
            if(resp.data){
              this._dialogRef.close();
            }
            else{
          ...
          error: (err: HttpErrorResponse) => {
            this._gfs.showLog('EditReleasdFunds', 'saveFunds', 'Error:', err);
            if(err.status == 401){
                this._ts.signOut();
                this._rtr.navigate(['/sign-in']);
            }
    ...
    getCurrencyCodes(): void {
        this._gfs.showLog('EditReleasdFunds', 'getCurrencyCodes', '', null);
        this._ms.getCurrencyCodes().subscribe({
          next: (orders: any) => {
            this._gfs.showLog('EditReleasdFunds', 'getCurrencyCodes', 'Response:', orders);
            this.currencyCodeList = orders.data;
    
          },
          error: (err: HttpErrorResponse) => {
            this._gfs.showLog('EditReleasdFunds', 'getCurrencyCodes', 'Error:', err);
            if(err.status == 401){
                this._ts.signOut();
                this._rtr.navigate(['/sign-in']);
            }
