...
export class ADCROpenComponent
{
    ...
    {
        this._gfs.showLog('ADCROpenComponent', 'constructor', '', null);
    ...
    ngAfterViewInit(): void {
        this._gfs.showLog('ADCROpenComponent', 'ngAfterViewInit', '', null);
    ...
    ngOnInit(): void {
        this._gfs.showLog('ADCROpenComponent', 'ngOnInit', '', null);
    ...
    ngOnDestroy(): void {
        this._gfs.showLog('ADCROpenComponent', 'ngOnDestroy', '', null);
    }

    resetSearchCriteria(): void {
        this._gfs.showLog('ADCROpenComponent', 'resetSearchCriteria', null, '');
    ...
    getListing(): void {
        this._gfs.showLog('ADCROpenComponent', 'getListing', '', null);
        
        if (this.userInfo && this.userInfo.email && this.userSession) {
            this._gfs.showLog('ADCROpenComponent', 'getListing', 'Get Information:', null);
            ...
            this._gfs.showLog('ADCROpenComponent', 'getListing', 'ELP:', elp);
            this._gfs.showLog('ADCROpenComponent', 'getListing', 'Get Information: Call Backend', null);

            // call the service
            this._els.getEscrowOrderList(elp).subscribe({
                next: (response: any) => {
                    this._gfs.showLog('ADCROpenComponent', 'getListing', 'Response:', response);

                    if(response.success && response.data && response.data.length > 0){
                        this._gfs.showLog('ADCROpenComponent', 'getListing', 'Get Escrow List Success:', null);
                ...
                error: (err: HttpErrorResponse) => {
                    this._gfs.showLog('ADCROpenComponent', 'getListing', 'Error:', err);
                    if(err.status == 401){
                        this._ts.signOut();
            ...
            this._gfs.showLog('ADCROpenComponent', 'getListing', 'BAD INFO:', null);
        }
    }

    showDetails(escrowOrderID: number): void {
        this._gfs.showLog('ADCROpenComponent', 'showDetails', 'escrowOrderID:', escrowOrderID);
        
        if(escrowOrderID > 0){
            this._gfs.showLog('ADCROpenComponent', 'showDetails', 'goingToOpen:', escrowOrderID);                       
