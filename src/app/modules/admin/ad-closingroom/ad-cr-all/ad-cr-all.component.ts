...
export class ADCRAllComponent
{
    ...
    {
        this._gfs.showLog('ADCRAllComponent', 'constructor', '', null);
    ...
    ngAfterViewInit(): void {
        this._gfs.showLog('ADCRAllComponent', 'ngAfterViewInit', '', null);
    ...
    ngOnInit(): void {
        this._gfs.showLog('ADCRAllComponent', 'ngOnInit', '', null);
    ...
    ngOnDestroy(): void {
        this._gfs.showLog('ADCRAllComponent', 'ngOnDestroy', '', null);
    }

    resetSearchCriteria(): void {
        this._gfs.showLog('ADCRAllComponent', 'resetSearchCriteria', null, '');
        this.submittedByID = this._ts.getUser().vectorsUID; // 128
        this.orderStatusID = 0; // not greater than 0 will get all??? 10 - open, 17 - closed
        this.checkedOutToID = 0;
    }

    getListing(): void {
        this._gfs.showLog('ADCRAllComponent', 'getListing', '', null);
        
        if (this.userInfo && this.userInfo.email && this.userSession) {
            this._gfs.showLog('ADCRAllComponent', 'getListing', 'Get Information:', null);
            ...
            this._gfs.showLog('ADCRAllComponent', 'getListing', 'ELP:', elp);
            this._gfs.showLog('ADCRAllComponent', 'getListing', 'Get Information: Call Backend', null);

            // call the service
            this._els.getEscrowOrderList(elp).subscribe({
                next: (response: any) => {
                    this._gfs.showLog('ADCRAllComponent', 'getListing', 'Response:', response);

                    if(response.success && response.data && response.data.length > 0){
                        this._gfs.showLog('ADCRAllComponent', 'getListing', 'Get Escrow List Success:', null);
                ...
                error: (err: HttpErrorResponse) => {
                    this._gfs.showLog('ADCRAllComponent', 'getListing', 'Error:', err);
                    if(err.status == 401){
                        this._ts.signOut();
            ...
            this._gfs.showLog('ADCRAllComponent', 'getListing', 'BAD INFO:', null);
        }
    }
