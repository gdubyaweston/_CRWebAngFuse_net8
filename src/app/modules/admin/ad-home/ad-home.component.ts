...
import { EscrowList, EscrowListSearch, EscrowParams } from 'app/modules/_modules/escrowclasses';
...
@Component({
    ...
    imports: [NgIf, CommonModule, MatPaginatorModule, MatSortModule, MatTableModule, MatTooltipModule],
})
export class ADHomeComponent {
    ...
    constructor(
        ...
        this._gfs.showLog('ADHomeComponent', 'constructor', '', null);
        this._gfs.allowCall = false;
        this.resertSearchCriteria();
        this.dataSource = new MatTableDataSource(this.daList);
    }

    ngAfterViewInit(): void {
        this._gfs.showLog('ADHomeComponent', 'ngAfterViewInit', '', null);
    ...
    ngOnInit(): void {
        this._gfs.showLog('ADHomeComponent', 'ngOnInit', '', null);
    ...
    ngOnDestroy(): void {
        this._gfs.showLog('ADHomeComponent', 'ngOnDestroy', '', null);
    }

    resertSearchCriteria(): void {
        this._gfs.showLog('ADHomeComponent', 'resertSearchCriteria', null, '');
        this.submittedByID = this._ts.getUser().vectorsUID; // 128
        this.orderStatusID = 10; // 10 - open, 17 - closed
        this.checkedOutToID = 0;
        
    }

    getListing(): void {
        this._gfs.showLog('ADHomeComponent', 'getListing', '', null);
        
        if (this.userInfo && this.userInfo.email && this.userSession) {
            this._gfs.showLog('ADHomeComponent', 'getListing', 'Get Information:', null);
            ...
            this._gfs.showLog('ADHomeComponent', 'getListing', 'ELP:', elp);
            this._gfs.showLog('ADHomeComponent', 'getListing', 'Get Information: Call Backend', null);

            // call the service
            this._els.getEscrowOrderList(elp).subscribe({
                next: (response: any) => {
                    this._gfs.showLog('ADHomeComponent', 'getListing', 'Response:', response);

                    if(response.success && response.data && response.data.length > 0){
                        this._gfs.showLog('ADHomeComponent', 'getListing', 'Get Escrow List Success:', null);
                        ...
                        this._gfs.showLog('ADHomeComponent', 'getListing', 'Get Escrow List Fail:', null);
                    }

                },
                error: (err: HttpErrorResponse) => {
                    this._gfs.showLog('ADHomeComponent', 'getListing', 'Error:', err);
                    if(err.status == 401){
                        this._ts.signOut();
            ...
            this._gfs.showLog('ADHomeComponent', 'getListing', 'BAD INFO:', null);
        }
    }

    showDetails(escrowOrderID: number): void {
        this._gfs.showLog('ADHomeComponent', 'showDetails', 'escrowOrderID:', escrowOrderID);
        
        if(escrowOrderID > 0){
            this._gfs.showLog('ADHomeComponent', 'showDetails', 'goingToOpen:', escrowOrderID);                       
